    ...
    <fullquery name="select_categories">
      <querytext>
        select child.keyword_id as child_id,
               ...
               case when child.keyword_id is null then 0 else (select count(*) from bt_contents where project_id = :package_id and content_keyword__is_assigned(content_id, child.keyword_id, 'none')) end as num_contents,
