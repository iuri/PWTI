...
-- Create the content content item object type
...
select define_function_args ('bt_content__new','content_id,content_number,package_id,component_id,found_in_version,summary,user_agent,comment_content,comment_formt,creation_date,creation_user,creation_ip,fix_for_version,item_subtype;bt_content,content_type;bt_content_revision');
...
begin
    ...
    insert into bt_contents
        (content_id, content_number, comment_content, comment_format, parent_id, project_id, creation_date, creation_user, fix_for_version)
    values
        (v_content_id, v_content_number, p_comment_content, p_comment_format, v_folder_id, p_package_id, p_creation_date, p_creation_user, p_fix_for_version);
...
-- There is a many to many relationship between patches and contents
create table bt_patch_content_map (
  patch_id            integer not null
                      constraint bt_patch_content_map_pid_fk
                      references bt_patches(patch_id)
                      on delete cascade,
  content_id              integer not null
                      constraint bt_patch_content_map_bid_fk
                      references cr_items(item_id)
                      on delete cascade,
  constraint bt_patch_content_map_un
  unique (patch_id, content_id)
);

create index bt_patch_content_map_patch_id_idx on bt_patch_content_map(patch_id);
create index bt_patch_content_map_content_id_idx on bt_patch_content_map(content_id);
