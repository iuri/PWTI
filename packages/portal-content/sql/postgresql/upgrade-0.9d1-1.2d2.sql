-- Upgrade script that converts the Portal Content to using the workflow package and the Content Repository
-- for its contents.
--
-- @author Lars Pind
...
-- has setup worklfow instances for the Portal Content package type and for all package instances

-- First move all workflow data for each content into the workflow data model
-- temporary table to map content tracker comment format to CR mime types
...
begin
  ...
  loop
    ...
    -- Get the content workflow id
    select workflow_id into v_workflow_id 
        from workflows 
        where short_name = ''content''
    ...
    for content_rec in select b.content_id, 
                          b.status, 
                          b.resolution, 
                          b.assignee, 
                          o.creation_user, 
                          o.creation_date
                   from bt_contents b, acs_objects o
                   where b.content_id = o.object_id
    ...
    loop
          ...
          insert into workflow_cases (case_id, workflow_id, object_id)
              values (v_case_id, v_workflow_id, content_rec.content_id);
          ...
          insert into workflow_case_role_party_map (case_id, role_id, party_id)
              values (v_case_id, v_submitter_role_id, content_rec.creation_user);

          -- Insert the assignee
          if content_rec.assignee is not null then
            select role_id into v_assignee_role_id 
              from workflow_roles 
              where short_name = ''assignee''
                and workflow_id = v_workflow_id;
            insert into workflow_case_role_party_map (case_id, role_id, party_id)
              values (v_case_id, v_assignee_role_id, content_rec.assignee);
          end if;

          -- Set the current state
          select state_id into v_current_state_id 
            from workflow_fsm_states
            where short_name = content_rec.status
              and workflow_id = v_workflow_id;
          ...
          for action_rec in select action, 
                                   ...
                                   comment, 
                                   comment_format
                            from bt_content_actions
                            where content_id = content_rec.content_id
          loop
              ...
              if action_rec.action = ''resolve'' then
                  insert into workflow_case_log_data (entry_id, key, value)
                          values (v_entry_id, ''resolution'', content_rec.resolution);
              end if;
          end loop;
...
-- remove the content-tracker notifications stuff completely
create function inline_0 ()
 ...
 declare
     ...
     row                             record;
 begin
     -- change content_tracker_project_notif to workflow
     select type_id
     into   v_old_notification_type_id
     from   notification_types 
     where  short_name = ''content_tracker_project_notif'';
     ...
     -- change content_tracker_content_notif to workflow_case
     select type_id
     into   v_old_notification_type_id
     from   notification_types 
     where  short_name = ''content_tracker_content_notif'';
     ...
     for row in select nt.type_id
                ...
                where nt.short_name in (''content_tracker_project_notif'', ''content_tracker_content_notif'')
     loop
...
-- Drop content tracker notifications. They are now taken care of by the workflow package
select bt_service_contract_delete('content_tracker_project_notif_type','content_tracker_project_notif');
select bt_service_contract_delete('content_tracker_content_notif_type','content_tracker_content_notif');
...
create table bt_contents_temp as select * from bt_contents;
...
begin
    ...
    v_folder_id := content_folder__new(
        ''content_tracker_''||p_package_id,        -- name
    ...
    PERFORM content_folder__register_content_type (
        ...
        ''bt_content_revision'',  -- content_type
        ''t''                 -- include_subtypes
    );
...
begin
    ...
    -- loop
    --     perform bt_content__delete(rec.item_id);
    -- end loop;
...
create table bt_contents(
  content_id                        integer
                                constraint bt_content_pk
                                primary key
                                constraint bt_content_bt_content_fk
  ...
  project_id                    integer,
  -- denormalized from bt_content_revisions
  ...
  -- constraint
  constraint bt_content_parent_id_content_number_un
  unique (parent_id, content_number)
);

-- LARS:
-- we need to figure out which ones of these will be used by the query optimizer

create index bt_contents_proj_id_content_number_idx on bt_contents(project_id, content_number);
create index bt_contents_content_number_idx on bt_contents(content_number);

create index bt_contents_proj_id_fix_for_idx on bt_contents(project_id, fix_for_version);
create index bt_contents_fix_for_version_idx on bt_contents(fix_for_version);

create index bt_contents_proj_id_crea_date_idx on bt_contents(project_id, creation_date);
create index bt_contents_creation_date_idx on bt_contents(creation_date);

-- Update the content content item object type
update acs_object_types set name_method = null where object_type = 'bt_content';


-- content_revision specialization
create table bt_content_revisions (
  content_revision_id               integer 
                                constraint bt_content_rev_pk
                                primary key
                                constraint bt_content_rev_content_id_fk
                                references cr_revisions(revision_id)
                                on delete cascade,
  component_id                  integer 
                                constraint bt_content_rev_components_fk
                                references bt_components(component_id),
  resolution                    varchar(50)
                                constraint bt_content_rev_resolution_ck
                                check (resolution is null or 
                                       resolution in ('fixed','bydesign','wontfix','postponed','duplicate','norepro','needinfo')),
  user_agent                    varchar(500),
  found_in_version              integer
                                constraint bt_content_rev_found_in_version_fk   
                                references bt_versions(version_id), 
  fix_for_version               integer
                                constraint bt_content_rev_fix_for_version_fk   
                                references bt_versions(version_id), 
  fixed_in_version              integer
                                constraint bt_content_rev_fixed_in_version_fk   
                                references bt_versions(version_id)
);

-- Create the content revision content type
...
select define_function_args ('bt_content__new','content_id,content_number,package_id,component_id,found_in_version,summary,user_agent,comment_content,comment_formt,creation_date,creation_user,creation_ip,item_subtype;bt_content,content_type;bt_content_revision');
...
begin
    ...
    v_content_id := content_item__new_temp(
...
-- ******* Migrate content type to CR keywords
create table content_type_keyword_map_temp (
   project_id   integer,
...
begin
    ...
    loop
        ...
        -- Create the content type root
        v_content_type_root := content_keyword__new (
            ''Bug Type'',
            null,  
            project_rec.root_keyword_id,
            null,
        ...
        insert into content_type_keyword_map_temp (project_id, content_type, keyword_id)
             values (project_rec.project_id, ''content'', v_keyword_id);
...
-- ******* START Bug upgrade
-- Create each of the contents with the new API that creates a content
-- item, an initial revision, and populates the bt_contents denormalization table
create or replace function inline_0 ()
returns integer as '
declare
  -- Project loop
...
begin
  ...
  loop
    ...
    loop
        ...
        -- Use a modified version of bt_content__new that doesn''t
        -- create a new acs_object
        perform bt_content__new (
            content_rec.content_id,     -- content_id
            content_rec.content_number,     -- content_number
            content_rec.project_id,     -- package_id
            content_rec.component_id,     -- component_id
            content_rec.found_in_version,     -- found_in_version
            content_rec.summary,     -- summary
            content_rec.user_agent,     -- user_agent
            content_rec.content,        -- comment_content
            content_rec.mime_type,     -- comment_format
            content_rec.creation_date,   -- creation_date
            content_rec.creation_user,     -- creation_user
            content_rec.creation_ip,     -- creation_ip
            ''bt_content'',     -- item_subtype
            ''bt_content_revision''      -- content_type               
        );
        ...
        from   cr_items
        where  item_id = content_rec.content_id;
        
        -- Update with fix_for_version, fixed_in_version, resolution
        update bt_content_revisions
        set    fix_for_version = content_rec.fix_for_version,
               fixed_in_version = content_rec.fixed_in_version,
               resolution = content_rec.resolution
        ...
        where  code_id = content_rec.severity;

        perform content_keyword__item_assign (
            content_rec.content_id,
            v_severity_id,
            null,
            content_rec.creation_user,
            content_rec.creation_ip
        );
        ...
        into   v_priority_id
        from   code_keyword_map_temp
        where  code_id = content_rec.priority;
        ...
        -- Map content type
        select keyword_id
        into   v_content_type_id
        from   content_type_keyword_map_temp
        where  project_id = content_rec.project_id
        and    content_type = content_rec.content_type;
...
drop table content_type_keyword_map_temp;
drop table bt_contents_temp;
drop table project_temp;

-- ******* Drop temporary upgrade functions
--drop function bt_content__new_temp(
--    integer,     -- content_id
--    integer,     -- content_number
--    integer,     -- package_id
--    integer,     -- component_id
