ad_library {
    ...
    @cvs-id $Id: file-storage-procs.tcl,v 1.58.2.1 2007/01/14 00:16:48 daveb Exp $
}
...
ad_proc fs_get_parent_folder_id {
    folder_id
} {
    Returns the name of a folder. 
} {
    return [db_list parent_folder_id "select folder_id from fs_folders where parent_id in ([join $folder_id ,])"]
}
...
ad_proc fs_get_folder_name {
    ...
    Returns parent folders ids of a folder. 
} {
...
ad_proc fs_get_folder_description {
    folder_id
} {
    Returns the name of a folder. 
} {
    return [db_string folder_name "select description
				       from cr_folders
   ...
   set package_and_root [fs::get_folder_package_and_root $folder_id]
    ...
    if {[string equal $action "new_file"] || [string equal $action "new_url"] || [string equal $action "new_version"]} {
        ...
        if {[string equal $action "new_version"]} {
            set sql "select description as description from cr_revisions where cr_revisions.revision_id = :item_id"
        } elseif {[string match "*folder" $action]} {
            set sql "select description from cr_folders where folder_id=:item_id"
        } else {
            set sql "select description as description from cr_revisions where cr_revisions.item_id = :item_id"
        }
    ...
    if {![string equal $community_id ""]} {
           ...
           append text_version "[_ file-storage.lt_Community_folder] <br>"
           set subject "$system_abreviation - [_ file-storage.files] - $community_name - $action_type"
    } else {
        set subject "$system_abreviation - [_ file-storage.files] - $action_type"
    }

		 
    set folder_name [lang::util::localize [fs_get_folder_name $folder_id]]
    append text_version "[_ file-storage.lt_File-Storage_folder_f]<br>"

    if {[string equal $action "new_version"]} {
        append text_version "[_ file-storage.lt_New_Version_Uploaded_]<br>"
    } else {
        append text_version "[_ file-storage.lt_Name_of_the_action_ty]<br>"
    }
    
    set system_abreviation [parameter::get_from_package_key -package_key kernel -parameter PublisherName]

    if {[info exists owner]} {
        append text_version "[_ file-storage.Uploaded_by_ownern]<br>"
    }
    if {[info exists description]} {
        append text_version "[_ file-storage.lt_Version_Notes_descrip]<br>"
    }

    append text_version "$hr <br><br>"
    
    set url_version "$url$path1?folder_id=$folder_id"
    append text_version "[_ file-storage.lt_View_folder_contents_]<br>"
    
    #set html_version [ad_html_text_convert -from text/plain -to text/html -- $text_version]
    set html_version $text_version
    append html_version "<br />"
    append html_version "$hr <br />"
    # Do the notification for the file-storage
    set users_notified {}
    set users_notified [notification::new \
			...
			-notif_text $text_version \
        		-notif_html $html_version \
                        -action_id $package_id \
			-already_notified $users_notified \
			-return_notified]


    # walk through all folders up to the root folder
    set folder_id_orig $folder_id
    while {$folder_id != $root_folder} {

	set parent_id [db_string parent_id "select parent_id from cr_items where item_id = :folder_id"]

	set users_notified [concat $users_notified [notification::new \
            			-type_id [notification::type::get_type_id \
                        	-short_name fs_fs_notif] \
            			-object_id $parent_id \
            			-notif_subject "$subject" \
