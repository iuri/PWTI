ad_library {
    ...
    @cvs-id $Id: file-storage-install-procs.tcl,v 1.12 2006/08/08 21:26:48 donb Exp $
...
ad_proc -public -callback pm::project_new -impl file_storage {
	...
	set folder_id [fs::new_folder \
			   -name $project_id \
			   -pretty_name "#file-storage.Folder_project#: $pm_name" \
...
ad_proc -public -callback pm::project_delete -impl file_storage {
    {-package_id:required}
    {-project_id:required}
} {
    delete folder for each project
} {
    set folder_ids [application_data_link::get_linked -from_object_id $project_id -to_object_type "content_folder"]
 
    foreach folder_id $folder_ids {
    	   set parent_id [db_string parent_id {select parent_id from cr_items where item_id = :folder_id} -default ""]
	   fs::delete_folder -folder_id $folder_id -parent_id $parent_id
	   application_data_link::delete_from_list -object_id $project_id -link_object_id_list $folder_id
    }
}
