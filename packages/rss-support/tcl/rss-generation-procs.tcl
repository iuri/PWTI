ad_library {
    ...
    @cvs-id $Id: rss-generation-procs.tcl,v 1.14 2005/01/13 13:58:40 jeffd Exp $
}
...
ad_proc rss_gen_200 {
    ...
    if {[llength $image] < 2 && ![empty_string_p $image]} {
		set base     $image 
        set url      $base
    ...
    foreach item $items {
        ...
        if { [exists_and_not_null iarray(description) ]} {
            append rss "<description><!\[CDATA\[$iarray(description)\]\]></description>" \n
        }
