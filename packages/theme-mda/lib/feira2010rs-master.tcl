...
set random_top [expr [randomRange 5]  + 1]

#backgound-color

switch $random_top {
	1 {set background_color "#5E5603"}
	2 {set background_color "#673407"}
	3 {set background_color "#1C0809"}
	4 {set background_color "#000"}
	default {set background_color "#fff"}
} 
...
<link rel="stylesheet" type="text/css" href="/resources/theme-mda/css/feira2010rs/estilos.css" media="screen">
...
<!\[endif\]-->
<!--\[if IE 6\]>
	<link href="/resources/theme-mda/css/feira2009/estilos_ie6.css" rel="stylesheet" type="text/css" />
	<style type="text/css">
	</style>
<!\[endif\]-->
<!--\[if IE 7\]>
	<link href="/resources/theme-mda/css/feira2009/estilos_ie7.css" rel="stylesheet" type="text/css" />
<!\[endif\]-->
<!--\[if IE 8\]>
	<link href="/resources/theme-mda/css/feira2009/estilos_ie8.css" rel="stylesheet" type="text/css" />
<!\[endif\]-->
...
for {set i 0} {$i < [llength $context_node_list_all]} {incr i} {
	if {$i != 0} {
...
set inferior_menu [menus::dropdown inferior $subsite_info(package_id)]
set superior_menu [menus::dropdown superior $subsite_info(package_id)]
...
set survey_arcoverde_admin [permission::permission_p -object_id 2343125 -party_id [ad_conn user_id] -privilege admin]
...
set videos_package_id [photo_album::get_package_id_from_url -url "${subsite_url}videos"]

# destaques com photo-album
set destaques_package_id [photo_album::get_package_id_from_url -url "${subsite_url}destaques"]

set destaques_url [site_node::get_url_from_object_id -object_id $destaques_package_id]
set pa_root_folder [pa_get_root_folder $destaques_package_id]
foreach album [photo_album::list_albums_in_root_folder -root_folder_id $pa_root_folder] {
		util_unlist $album album_name album_id
		append photos_on_page "[pa_all_photos_in_album_internal $album_id] "
}

if {[exists_and_not_null photos_on_page] && $photos_on_page != " "} {
	set photos_on_page_sql "and ci.item_id in ([join $photos_on_page ,])"
} else {
	set photos_on_page_sql ""
}

if {[exists_and_not_null photos_on_page_sql]} {
	db_multirow child_photo child_photo "
      select ci.item_id as photo_id,
      (select pp.caption from pa_photos pp where pp.pa_photo_id = ci.live_revision) as caption,
      (select pp.story from pa_photos pp where pp.pa_photo_id = ci.live_revision) as story,
	  cr.description as link,
	...
	and ci.parent_id = 4104669
      $photos_on_page_sql
	order by photo_id desc
	limit 1
	" {
	    #set story [util_close_html_tags $story "110" "110" "..." ""]
	    #set story [ad_text_to_html $story]
	}
} else {
	template::multirow create child_photo photo_id caption story thumb_path thumb_height thumb_width album_id
}

if {$portal_page_p == 1} {
	#template::add_body_handler -event onload -script "javascript:destaque();"
}


ah::requires -sources "jquery"
#query
template::head::add_javascript -src "/resources/theme-mda/js/feira2010rs/upbox.js" -order 5
template::head::add_javascript -script "query=\"from=feiramda\"" -order 6
