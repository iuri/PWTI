...
<div class=\"relatorio\">
	<div class=\"left\" style=\"float: left;width: 45%;\">
		...
		<ul>
			...
			<li> <b>Menções Únicas nas Redes: </b>$qtd_tota@ <br /></li>
...
set filename_bs [ns_mktemp "/tmp/htmlXXXX"]
set filename_pdf "teste-${filename_bs}.pdf"
ns_log notice "file names $filename_pdf  ${filename_base}.html"
...
set filename_base2 [ns_mktemp "/tmp/htmlXXXXXXXX"]
exec iconv -t iso8859-1 ${filename_base}.html > $filename_base2
ns_log notice "passou iconv" 
if {[catch {exec htmldoc --firstpage toc --size 210x297mm --left 1cm --right 1.5cm --webpage --quiet -f $filename_pdf $filename_base2 -} errmsg]} {
    ...
    if { $errmsg eq "child process exited abnormally"} {
       	ns_returnfile 200 application/pdf $filename_pdf
    } else {
		set report_content $errmsg
    }
	file delete -force -- "${filename_base}.html"
} else {
