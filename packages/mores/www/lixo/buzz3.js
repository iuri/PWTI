...
document.write(unescape("%3Cscript src='/engine.js' type='text/javascript'%3E%3C/script%3E"));
...
var smSources = (typeof smSources == 'undefined' || isArray(smSources) == false ?
	['twitter', 'googleblog', 'brightkite', 'delicious', 'friendfeed', 'flickr', 'identica', 'youare','googleblog2', 'digg'] : smSources);
...
var simpleSpy = function(iframe_container_id, smSearchPhrase, smShowUserImages, smFontSize, smTitle, smSources) {
	...
	this.insertCSS = function() {
		...
		css.setAttribute("href", "/buzz.css");
...
//			this.play = 0;
	...
	if(item.user && this.smShowUserImages == true) {
		...
		var div4 = Helper.createNode('div', {"className":"source"}, {}, '<img src="' + item.favicon + '" />&nbsp;' +
			'<span class="date" rel="' + item.timestamp + '"><a title="Veja o post original " href="' + item.url +  '" target="_blank">' + timeAgo(item.timestamp) + '</a></span> ' + (item.user ? ' por <a href="' + item.profile + '" target="_blank">'+ item.user + '</a>' : '') + ' - ' + item.source +' <a class="link_original" href="' + item.url  + '" target="_blank"> Veja o Post Completo e comente</a>', this.get_iframe().contentWindow.document);
